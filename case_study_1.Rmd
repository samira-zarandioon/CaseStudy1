...
In this report we analyze GDP ranking and distribution of 180 countries. Also, we study the relationship of the GDP with income groups.
 
To perform the analysis, using a make file, we first download the Gross Domestic product and the educationl data sets and after the clean up we analyze the tidy data. These steps and the analysis results are documented in following sections.
...
cat("\nRows with missing GDP value = ", nrow(GDP_dataframe[is.na(GDP_dataframe$GDP),]), "\n" )
cat("Rows with missing Ranked value = ", nrow(GDP_dataframe[is.na(GDP_dataframe$Rank),]), "\n" )
...
3. Download the educationl data set from [here](https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FEDSTATS_Country.csv) and clean the data by removing the 24 rows with missing income group and keeping only the relevat columns (i.e. CountryCode and Income.Group). The clean data set has 210 rows.
...
cat("Rows in the initial ED data = ", nrow(ED_dataframe), "\n")
...
write.csv(ED_dataframe, file="ED_dataframe.csv")

```

4. Merge the data set containig GDP info with the data set containing info about income group by matching the country codes.
...
sorted_GDP_IncomeGroup_dataframe <- GDP_IncomeGroup_dataframe[with(GDP_IncomeGroup_dataframe,order(GDP, na.last = TRUE)),]
...
ggplot(GDP_IncomeGroup_dataframe, aes(x=reorder(CountryCode, GDP), y=GDP/10^6)) +
  geom_bar(aes(fill=Income.Group),   # fill depends on cond2
           stat="identity",
